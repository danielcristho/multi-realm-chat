...
class Chat:
    ...
    def send_file_private(self, sessionid, username_from, username_dest, file_path):
        if sessionid not in self.sessions:
            return {'status': 'ERROR', 'message': 'Session Tidak Ditemukan'}
        ...
        if s_fr['realm'] != s_to['realm']:
            # Handle pengiriman file cross-realm
            return self.send_cross_realm_file(s_fr['realm'], s_to['realm'], username_from, username_dest, file_path)
        
        file_data = open(file_path, 'rb').read()
        message = {'msg_from': s_fr['nama'], 'msg_to': s_to['nama'], 'file': file_data}
    ...
    def send_cross_realm_file(self, realm_from, realm_to, username_from, username_dest, file_path):
        try:
            logging.info(f"Attempting to connect to {realm_to}:8889")
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
                sock.connect((realm_to, 8889))
            ...
            logging.error(f"Error mengirimkan file cross-realm: {e}")
            return {'status': 'ERROR', 'message': f'Error mengirimkan file cross-realm: {str(e)}'}

    def send_file_group(self, sessionid, username_from, groupname, file_path):
        if groupname not in self.groups:
            return {'status': 'ERROR', 'message': 'Grup tidak ditemukan'}
        ...
        group_message = {'msg_from': username_from, 'file': file_path}
        self.groups[groupname]['messages'].put(group_message)
        
        return {'status': 'OK', 'message': 'File grup terkirim'}

    def get_group_file(self, sessionid, groupname):
        if groupname not in self.groups:
            return {'status': 'ERROR', 'message': 'Grup tidak ditemukan'}
...
# Mengirimkan file secara privat
